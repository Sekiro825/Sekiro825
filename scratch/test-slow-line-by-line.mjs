...
// 4. Line-by-Line Row Assembly (Group into 2-row block bands for ultra-smooth progression)
const BAND_ROWS = 2; // 2 rows per falling line block band -> 38 bands total
const NUM_BANDS = Math.ceil(ROWS / BAND_ROWS);

let lineBlocksSvg = "";
const TOTAL_DUR = 14; // 14 seconds slow hypnotic loop

for (let b = 0; b < NUM_BANDS; b++) {
  const rStart = b * BAND_ROWS;
  const rEnd = Math.min(ROWS, rStart + BAND_ROWS);
  ...
  for (let r = rStart; r < rEnd; r++) {
    for (let c = 0; c < COLS; c++) {
      ...
      if (ch !== " ") {
        ...
        bandContent += `<tspan x="${x.toFixed(2)}" y="${y.toFixed(1)}" fill="${color}" opacity="${opacity}">${ch}</tspan>`;
      }
    }
  }

  if (hasContent) {
    // STRICT BOTTOM-TO-TOP LINE-BY-LINE DELAY:
    // b = NUM_BANDS - 1 is the bottommost row band (suit) -> ratio = 0 -> delay = 0s
    // b = 0 is the topmost row band (hair) -> ratio = 1 -> delay = 6.2s
    const bottomToTopRatio = (NUM_BANDS - 1 - b) / (NUM_BANDS - 1);
    const delay = bottomToTopRatio * 6.2;

    lineBlocksSvg += `  <g class="line-fall" style="animation-delay: ${delay.toFixed(2)}s;">\n`;
    lineBlocksSvg += `    <text class="ascii-font">${bandContent}</text>\n`;
    lineBlocksSvg += `  </g>\n`;
  }
}

console.log(`Generated ${NUM_BANDS} line block bands for slow bottom-to-top assembly.`);

// 5. Update dark.svg
let darkSvg = fs.readFileSync('dark.svg', 'utf8');

const newStyles = `
    @keyframes slowLineFall {
      0% {
    ...
    .line-fall {
      animation: slowLineFall ${TOTAL_DUR}s cubic-bezier(0.22, 1, 0.36, 1) infinite;
    }
    .ascii-font {
...
const newViewportContent = `<g id="visual-map-viewport" class="viewport-clip">
    ...
    <!-- Slow Smooth Line-by-Line Bottom-to-Top Assembly -->
${lineBlocksSvg}
...
console.log('Successfully updated dark.svg with slow smooth line-by-line bottom-to-top assembly!');
