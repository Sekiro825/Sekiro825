...
for (let r = 0; r < ROWS; r++) {
  ...
  for (let c = 0; c < COLS; c++) {
    ...
    if (!isBgPixel) {
      ...
      const contrastNorm = Math.pow(norm, 1.3); 
...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1180" height="610" viewBox="0 0 1180 610">
  ...
  <text font-family="'Courier New', Consolas, monospace" font-size="${(lineHeight*0.9).toFixed(1)}px" font-weight="bold" fill="#38BDF8" letter-spacing="${(charSpacing*0.1).toFixed(1)}px">
${asciiRowsSvg}
  </text>
</svg>`;

fs.writeFileSync('scratch/test-numbers2.svg', svg);
console.log('Done');
