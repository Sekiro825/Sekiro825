...
// 4. Divide into Tetris Block Chunks (5 cols x 4 rows per block)
const BLOCK_W = 5;
const BLOCK_H = 4;
...
let tetrisBlocksSvg = "";
const TOTAL_DUR = 12; // 12 seconds slow, smooth loop
...
for (let br = 0; br < NUM_BLOCK_ROWS; br++) {
  for (let bc = 0; bc < NUM_BLOCK_COLS; bc++) {
    ...
    if (hasContent) {
      // SMOOTH SLOW BOTTOM TO TOP ASSEMBLY:
      // br = NUM_BLOCK_ROWS - 1 (bottom suit) -> ratio = 0 -> delay = 0.0s (LANDS FIRST)
      // br = 0 (top hair) -> ratio = 1 -> delay = 4.2s (LANDS LAST)
      const bottomToTopRatio = (NUM_BLOCK_ROWS - 1 - br) / (NUM_BLOCK_ROWS - 1);
      
      const hSeed = (Math.sin(bc * 17.3 + br * 7.1) * 23421.123) % 1;
      const delay = bottomToTopRatio * 4.2 + Math.abs(hSeed) * 0.35;
...
console.log(`Generated ${NUM_BLOCK_COLS * NUM_BLOCK_ROWS} Tetris block chunks with slow bottom-to-top progression.`);
...
const newStyles = `
    @keyframes tetrisBlockFall {
      0% {
        transform: translateY(-460px) scale(0.92);
        opacity: 0;
      }
      10% {
        opacity: 0.9;
    ...
    .tetris-block {
      animation: tetrisBlockFall ${TOTAL_DUR}s cubic-bezier(0.18, 1, 0.32, 1) infinite;
    }
    .ascii-font {
...
const newViewportContent = `<g id="visual-map-viewport" class="viewport-clip">
    ...
    <!-- Slow Smooth Tetris Block Bottom-to-Top Assembly -->
${tetrisBlocksSvg}
...
console.log('Successfully restored Tetris block chunks with slow & smooth bottom-to-top assembly!');
